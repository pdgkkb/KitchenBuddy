...
class OpenAICompatLLM:
    ...
    def _budget_for(self, want: int) -> int:
        """The most tokens that can actually arrive before the clock runs out.

        A budget is a promise about the worst case: if the model uses all of
        it, the wait is budget / rate seconds. When that exceeds the timeout,
        the request is arithmetically impossible and raising the budget — the
        obvious response to a truncated recipe — makes it worse, not better.
        Ten per cent is left for the prompt and the connection.
        """
    ...
    async def json(self, system: str, user: str, schema: dict, max_tokens: int = 2400) -> dict:
        ...
        max_tokens = self._budget_for(max(max_tokens, self.json_tokens))
        resp = await self._create(
